...
type UpdateCalculationRequest struct {
	Calculations []ds.UpdateCalculationReadIndxs `json:"calculations"`
}

func (h *Handler) API_ReadIndxsUpdateCalcs(c *gin.Context) {
	var body ds.UpdateCalculationReadIndxs
	if err := c.BindJSON(&body); err != nil {
		h.errorHandler(c, 400, err)
		return
	}
	if c.Request.Header.Get("Authorization") != h.Config.AuthToken {
		h.errorHandler(c, 403, fmt.Errorf("invalid auth token"))
		return
	}
	if err := h.Repository.ReadIndxsUpdateCalcs(body); err != nil {
		h.errorHandler(c, 500, err)
		return
	}
	c.Status(200)
